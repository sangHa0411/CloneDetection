...
def add_entity_mask(
  ...
  premise_mask[
    first_token_start_index : first_token_end_index + 1
  ] = 1

  hypothesis_mask[
    second_token_start_index : second_token_end_index + 1
  ] = 1
        ...
        if self.model_category == "codebert":
            ...
            for i in range(batch_size):
                ...
                if len_input_ids >= self.max_input_length:
                    ...
                    code1_length_bart, code2_length_bart = len(code1_input_ids)/(code_max_length), len(code2_input_ids)/(code_max_length)
